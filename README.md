# Iban-del

> Étudiant en BUT Informatique | Développeur d'applications

---

##  aboutMe

> Actuellement âgé de 18 ans, je suis développeur d'applications chez **Network Informatique**  
> et étudiant à l'**IUT de Laval** en **BUT Informatique**.  
> Passionné par le développement et les nouvelles technologies, je suis en apprentissage constant.  
> Curieux et motivé, je découvre et maîtrise continuellement de nouveaux outils, langages et environnements.

---

## Skills

**Langages & Frameworks**

![JS](https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript)
![TS](https://img.shields.io/badge/TypeScript-black?style=for-the-badge&logo=typescript)
![React](https://img.shields.io/badge/React-black?style=for-the-badge&logo=react)
![ExtJs](https://img.shields.io/badge/ExtJS-black?style=for-the-badge&logo=sencha)
![HTML](https://img.shields.io/badge/HTML-black?style=for-the-badge&logo=html5)
![Tailwind](https://img.shields.io/badge/Tailwind-black?style=for-the-badge&logo=tailwindcss)
![CSS](https://img.shields.io/badge/CSS-black?style=for-the-badge&logo=css3)
![PHP](https://img.shields.io/badge/PHP-black?style=for-the-badge&logo=php)
![Python](https://img.shields.io/badge/Python-black?style=for-the-badge&logo=python)
![C](https://img.shields.io/badge/C-black?style=for-the-badge&logo=c)
![SQL](https://img.shields.io/badge/SQL-black?style=for-the-badge&logo=mysql)
![Redis](https://img.shields.io/badge/Redis-black?style=for-the-badge&logo=redis)
![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)

---

## Projects

| Projet | Description |
|-------|-------------|
|  **Labyrinthe** | Génération et résolution automatique de labyrinthes avec l’algorithme **A\*** en Python |
|  **Gestion des enseignants** | Gestion de plannings, cours et affectations avec technologies web |
|  **Cœur Applicatif** | Socle applicatif modulaire pour accélérer la création de projets |
|  **Portfolio** | Site web personnel moderne et fluide |
|  **ServerRoomFan** | Système intelligent pour surveiller et réguler la température d’une salle serveur |
|  **Interface d’images** | Application permettant l’édition et la comparaison d’images avec filtres |

---

## Experience

**Network Informatique**  
> Contrat développeur d'application

**Stage Développeur d'application**  
- Système de refroidissement autonome  
- Interface de gestion de traduction

---

## Studies

-  **BUT Informatique — IUT de Laval**
-  **Bac Pro Systèmes Numériques (2022-2025)**

---

## Activities_passions

- Apprendre en continu  
- Développer tout et n'importe quoi  
- Univers **Cyberpunk**  
- **Machine Learning**

---

## Contact

-  GitHub : https://github.com/Iban-del  
-  LinkedIn : iban-deletoille-elizalde  
-  Mail : deletoilleelizaldeiban@gmail.com  
-  Discord : @Bouboune
-  Portfolio : https://iban-del.github.io/PortFolio/

---

